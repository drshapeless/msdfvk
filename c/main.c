...
// Here I put all the msdf library code in a single file.
 ...
 */
int ex_msdf_glyph_mem(stbtt_fontinfo *font,
                      uint32_t c,
                      size_t w,
                      size_t h,
                      float *bitmap,
                      ...
                      int autofit);
float *ex_msdf_glyph(stbtt_fontinfo *font,
                     uint32_t c,
                     size_t w,
                     size_t h,
                     ex_metrics_t *metrics,
...
signed_distance_t
quadratic_dist(edge_segment_t *e, vec2 origin, double *param) {
    vec2 qa, ab, br;
    ...
    *param = -vec2_mul_inner(qa, ab) / vec2_mul_inner(ab, ab);
    ...
    if (*param < .5)
        return (signed_distance_t){ min_distance,
                                    fabs(vec2_mul_inner(ab, qa)) };
    else
        return (signed_distance_t){ min_distance,
                                    fabs(vec2_mul_inner(aa, bb)) };
}
    ...
    *param = -vec2_mul_inner(qa, ep_dir) / vec2_mul_inner(ep_dir, ep_dir);
    ...
    for (int i = 0; i <= search_starts; ++i) {
        ...
        for (int step = 0;; ++step) {
            ...
            t -= vec2_mul_inner(qpt, d1) /
                 (vec2_mul_inner(d1, d1) + vec2_mul_inner(qpt, d2));
            if (t < 0 || t > 1)
                break;
        }
    }
    ...
    if (*param < .5)
        return (signed_distance_t){ min_distance,
                                    fabs(vec2_mul_inner(d0, qa)) };
    else
        return (signed_distance_t){ min_distance, fabs(vec2_mul_inner(d1, a)) };
}

void dist_to_pseudo(signed_distance_t *distance,
                    vec2 origin,
                    double param,
                    edge_segment_t *e) {
    if (param < 0) {
...
void switch_color(edge_color_t *color,
...
void linear_split(edge_segment_t *e,
                  edge_segment_t *p1,
                  edge_segment_t *p2,
                  edge_segment_t *p3) {
    vec2 p;
...
void quadratic_split(edge_segment_t *e,
                     edge_segment_t *p1,
                     edge_segment_t *p2,
                     edge_segment_t *p3) {
    vec2 p, a, b;
...
void cubic_split(edge_segment_t *e,
                 edge_segment_t *p1,
                 edge_segment_t *p2,
                 edge_segment_t *p3) {
    vec2 p, a, b, c, d;
...
void edge_split(edge_segment_t *e,
                edge_segment_t *p1,
                edge_segment_t *p2,
                edge_segment_t *p3) {
    switch (e->type) {
...
int ex_msdf_glyph_mem(stbtt_fontinfo *font,
    ...
    float xoff = .5, yoff = .5;
    stbtt_GetGlyphBox(
        font, stbtt_FindGlyphIndex(font, c), &ix0, &iy0, &ix1, &iy1);
    ...
    stbtt_GetGlyphHMetrics(
        font, stbtt_FindGlyphIndex(font, c), &advance, &left_bearing);
    left_bearing *= scale;
    ...
    stbtt_vertex *verts;
    int num_verts =
        stbtt_GetGlyphShape(font, stbtt_FindGlyphIndex(font, c), &verts);
    ...
    for (int i = 0; i < contour_count; ++i) {
        if (contour_data[i].edge_count > 0) {
            vec2 prev_dir, dir;
            direction(prev_dir,
                      &contour_data[i].edges[contour_data[i].edge_count - 1],
                      1);
        ...
        if (corner_index == 0) {
            ...
            if (contour_data[i].edge_count >= 3) {
                ...
                for (int j = 0; j < m; ++j)
                    contour_data[i].edges[(corner + j) % m].color =
                        (colors +
                         1)[(int)(3 + 2.875 * i / (m - 1) - 1.4375 + .5) - 3];
            } else if (contour_data[i].edge_count >= 1) {
                edge_segment_t *parts[7] = { NULL };
                edge_split(&contour_data[i].edges[0],
                           parts[0 + 3 * corner],
                           parts[1 + 3 * corner],
                           parts[2 + 3 * corner]);
                if (contour_data[i].edge_count >= 2) {
                    edge_split(&contour_data[i].edges[1],
                               parts[3 - 3 * corner],
                               parts[4 - 3 * corner],
                               parts[5 - 3 * corner]);
                ...
                for (int j = 0; parts[j]; ++j) {
                    memcpy(&contour_data[i].edges[index++],
                           &parts[j],
                           sizeof(edge_segment_t));
                    contour_data[i].edge_count++;
                }
            }
        } else {
            ...
            for (int j = 0; j < m; ++j) {
                ...
                if (spline + 1 < corner_count && corners[spline + 1] == index) {
                    ...
                    edge_color_t s =
                        (edge_color_t)((spline == corner_count - 1) *
                                       initial_color);
    ...
    ///// Debug print contour
    for (int i = 0; i < contour_count; i++) {
        printf("contour %d: edge count: %zu\n", i, contour_data[i].edge_count);
        for (int j = 0; j < contour_data[i].edge_count; j++) {
            printf("edge %d: ", j);
            print_edge_segment(contour_data[i].edges[j]);
        }
    }
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            vec2 p = { (x + xoff - translate_x) / (scale * 64.0),
            ...
            for (int j = 0; j < contour_count; ++j) {
                ...
                r.min_distance.dist = g.min_distance.dist =
                    b.min_distance.dist = INF;
                ...
                for (int k = 0; k < contour_data[j].edge_count; ++k) {
                    ...
                    if (e->color & RED &&
                        signed_compare(distance, r.min_distance)) {
                        r.min_distance = distance;
                        r.near_edge = e;
                        r.near_param = param;
                    }
                    if (e->color & GREEN &&
                        signed_compare(distance, g.min_distance)) {
                        g.min_distance = distance;
                        g.near_edge = e;
                        g.near_param = param;
                    }
                    if (e->color & BLUE &&
                        signed_compare(distance, b.min_distance)) {
                ...
                double med_min_dist = fabs(median(r.min_distance.dist,
                                                  g.min_distance.dist,
                                                  b.min_distance.dist));
                ...
                if (r.near_edge)
                    dist_to_pseudo(
                        &r.min_distance, p, r.near_param, r.near_edge);
                if (g.near_edge)
                    dist_to_pseudo(
                        &g.min_distance, p, g.near_param, g.near_edge);
                if (b.near_edge)
                    dist_to_pseudo(
                        &b.min_distance, p, b.near_param, b.near_edge);

                med_min_dist = median(r.min_distance.dist,
                                      g.min_distance.dist,
                                      b.min_distance.dist);
                ...
                if (windings[j] > 0 && med_min_dist >= 0 &&
            ...
            if (sr.near_edge)
                dist_to_pseudo(
                    &sr.min_distance, p, sr.near_param, sr.near_edge);
            if (sg.near_edge)
                dist_to_pseudo(
                    &sg.min_distance, p, sg.near_param, sg.near_edge);
            if (sb.near_edge)
                dist_to_pseudo(
                    &sb.min_distance, p, sb.near_param, sb.near_edge);
            ...
            for (int i = 0; i < contour_count; ++i)
                if (windings[i] != winding &&
                    fabs(contour_sd[i].med) < fabs(msd.med))
                    msd = contour_sd[i];

            if (median(sr.min_distance.dist,
                       sg.min_distance.dist,
                       sb.min_distance.dist) == msd.med) {
    ...
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            if ((x > 0 && pixel_clash(P(x, y, w, bitmap),
                                      P(MAX(x - 1, 0), y, w, bitmap),
                                      tx)) ||
                (x < w - 1 && pixel_clash(P(x, y, w, bitmap),
                                          P(MIN(x + 1, w - 1), y, w, bitmap),
                                          tx)) ||
                (y > 0 && pixel_clash(P(x, y, w, bitmap),
                                      P(x, MAX(y - 1, 0), w, bitmap),
                                      ty)) ||
                (y < h - 1 && pixel_clash(P(x, y, w, bitmap),
                                          P(x, MIN(y + 1, h - 1), w, bitmap),
                                          ty))) {
                clashes[cindex].x = x;
                clashes[cindex++].y = y;
            }
...
int main(int argc, char *argv[]) {
    ...
    stbtt_fontinfo font;
    stbtt_InitFont(
        &font, (const uint8_t *)data, stbtt_GetFontOffsetForIndex(data, 0));

    // generate a msdf bitmap
    // ideally you would do this in your shader
    ex_metrics_t metrics;
    float *msdf =
        ex_msdf_glyph(&font, my_char, size_sdf, size_sdf, &metrics, 1);
